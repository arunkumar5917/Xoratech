...
export default function StudentDashboard() {
  ...
  return (
    <section className="min-h-screen pt-24 pb-16 lg:pt-28">
      <div className="container-x">
        {/* Header */}
        <FadeIn direction="up">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-xora-500 dark:text-xora-400">Student Portal</p>
              <h1 className="mt-1 font-display text-2xl font-bold text-navy-950 dark:text-white sm:text-3xl">
                Welcome, Student
              </h1>
            </div>
            <div className="flex items-center gap-3">
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-navy-100 dark:border-navy-800 bg-white/80 dark:bg-navy-900/80 px-3.5 py-2 text-sm font-medium text-navy-700 dark:text-navy-300 transition-all hover:bg-white dark:hover:bg-navy-800 shadow-sm hover:-translate-y-0.5"
              >
                <Award className="h-4 w-4 text-xora-500" />
                Verify Certificate
              </Link>
              <button className="inline-flex items-center gap-1.5 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50/80 dark:bg-red-950/40 px-3.5 py-2 text-sm font-medium text-red-600 dark:text-red-400 transition-all hover:bg-red-100 dark:hover:bg-red-950/80 shadow-sm active:scale-95">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-4">
          {/* Sidebar */}
          <FadeIn direction="right" delay={0.1} className="lg:col-span-1">
              ...
              {sidebarItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition-all text-left ${
                      active
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <FadeIn direction="left" delay={0.15}>
              <div className="rounded-3xl border border-navy-100 dark:border-navy-800 bg-white/80 dark:bg-navy-900/80 backdrop-blur-md p-6 shadow-card">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold text-xora-600 dark:text-xora-400">65% Complete</span>
                </div>
                <div className="mt-4">
                  <AnimatedProgressBar value={65} />
                  <p className="mt-2 text-xs text-navy-500 dark:text-navy-400">
                    You&apos;ve completed 5 of 8 modules. Keep up the great work!
                  </p>
                </div>
              </div>
            </FadeIn>
            ...
            {/* Quick Stats */}
            <StaggerContainer staggerDelay={0.06} className="grid gap-4 sm:grid-cols-2">
              {quickStats.map((stat) => (
                <StaggerItem key={stat.label}>
                  <HoverCard lift={-3} className="h-full">
                    <div className="flex h-full items-center gap-3.5 rounded-3xl border border-navy-100 dark:border-navy-800 bg-white/80 dark:bg-navy-900/80 backdrop-blur-md p-5 shadow-card">
              ...
              <div className="rounded-3xl border border-navy-100 dark:border-navy-800 bg-white/80 dark:bg-navy-900/80 backdrop-blur-md p-6 shadow-card">
                ...
                <div className="mt-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-navy-400 dark:text-navy-500">Assigned to your active cohort</p>
                  </div>
