...
export function Button({
  ...
  const base = cn(
    "inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-xora-500 focus-visible:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed",
    {
      ...
      "border-2 border-navy-100 bg-white text-navy-900 hover:border-xora-500 hover:text-xora-600": variant === "outline",
      "bg-navy-900 text-white shadow-card hover:bg-navy-800 hover:-translate-y-0.5": variant === "navy",
      "bg-white text-navy-900 shadow-card hover:shadow-xl hover:-translate-y-0.5": variant === "white",
      "border-2 border-white/30 bg-white/10 text-white hover:border-xora-400 hover:bg-white/20 backdrop-blur-sm": variant === "outline-light",
    },
    className
  );
